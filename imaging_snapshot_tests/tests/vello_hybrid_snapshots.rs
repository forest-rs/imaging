...
fn render_case(case: &dyn imaging_snapshot_tests::cases::SnapshotCase) -> Image {
    ...
    let mut renderer = VelloHybridRenderer::try_new(width, height).expect("create vello_hybrid");
    let bytes = renderer
...
fn snapshots() {
    ...
    if VelloHybridRenderer::try_new(DEFAULT_WIDTH, DEFAULT_HEIGHT).is_err() {
        eprintln!("[vello_hybrid] no adapter/device available; skipping snapshots");
        return;
    }
    let mut errors = Vec::new();
    common::run_cases("vello_hybrid", render_case, &mut errors);
